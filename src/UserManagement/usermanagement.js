...
app.post('/verifyuser', async (req, res) => {
    ...
    const query = niql.fromString(
        "select " + bucket._name + " `user` from " + bucket._name +
    ...
    try {
        await bucket.query(query, [name, password], (err, row) => {
            ...
            else if (row == undefined || row == null || row.length <= 0) res.send(row)
...
app.post('/updateuserdata', async (req, res) => {
    ...
    const query = niql.fromString(
        "UPDATE " + bucket._name + " USE KEYS $1" +
        "set name=$2,`password`=$3,email= $4,phone= $5" +
        "where type='user'"
    );
    try {
        await bucket.query(query, [userid, name, password, email, phone], async (err, row) => {
            if (err) {
                throw err;
            } else {
